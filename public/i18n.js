...
(function (global) {
  ...
  CATALOG.en = {
    ...
    'conn.stale':          'Waiting for reply ({s}s)',
    'conn.silent':         'No reply for {s}s',
    'conn.timeout':        'Timed out',
    'conn.live_error':     'Connected · live readings failing',
    'conn.tip':            'Last reply {at} · server pid {pid} (up since {boot}) · beat #{seq}',
    'conn.tip_live_error': 'Reading Claude Code\'s records failed: {msg}',
    ...
    'live.bay_note_ended':  'They ran without ever being recorded and have already finished (read from Claude Code records). Their launcher could not be measured either.',
    'live.bay_show':       'Show',
    'live.bay_hide':       'Hide',
    'live.orphan_parent':  'spawned by {name}',
    'card.no_mission':     '(no mission recorded)',
    'card.detected':       'No report — finish detected from Claude Code\'s records',
    'card.detected_stopped': 'No report — stop ({status}) detected from Claude Code\'s records',
  ...
  CATALOG.ja = {
    ...
    'conn.silent':         '{s}秒応答なし',
    'conn.timeout':        '時間切れ',
    'conn.live_error':     '接続中・実測の読み取りに失敗',
    'conn.tip':            '最終受信 {at}・サーバー pid {pid}（{boot} 起動）・拍 #{seq}',
    'conn.tip_live_error': 'Claude Code の記録の読み取りに失敗: {msg}',
    ...
    'live.bay_note_ended':  '記録に無いまま動いて、すでに終わった機体です（Claude Code の記録から実測）。起動元も実測で辿れませんでした。',
    'live.bay_show':       '表示',
    'live.bay_hide':       '隠す',
    'live.orphan_parent':  '起動元 {name}',
    'card.no_mission':     '（任務未記載）',
    'card.detected':       '報告なし — Claude Code の記録から完了を検知',
    'card.detected_stopped': '報告なし — Claude Code の記録から停止（{status}）を検知',
  ...
  CATALOG.zh = {
    ...
    'conn.silent':         '{s}秒无响应',
    'conn.timeout':        '超时',
    'conn.live_error':     '已连接 · 实测读取失败',
    'conn.tip':            '最后接收 {at} · 服务器 pid {pid}（{boot} 启动）· 心跳 #{seq}',
    'conn.tip_live_error': '读取 Claude Code 记录失败：{msg}',
    ...
    'live.bay_note_ended':  '在没有记录的情况下运行、且已经结束的机体（根据 Claude Code 的记录实测）。启动来源也无法实测。',
    'live.bay_show':       '显示',
    'live.bay_hide':       '隐藏',
    'live.orphan_parent':  '由 {name} 启动',
    'card.no_mission':     '（未填写任务）',
    'card.detected':       '无报告 — 从 Claude Code 的记录检测到完成',
    'card.detected_stopped': '无报告 — 从 Claude Code 的记录检测到停止（{status}）',
  ...
  CATALOG.ko = {
    ...
    'conn.broken':         '화면 오류',
    'conn.stale':          '응답 대기({s}초)',
    'conn.silent':         '{s}초 응답 없음',
    'conn.timeout':        '시간 초과',
    'conn.live_error':     '연결됨 · 실측 읽기 실패',
    'conn.tip':            '마지막 수신 {at} · 서버 pid {pid}({boot} 시작) · 박동 #{seq}',
    'conn.tip_live_error': 'Claude Code 기록 읽기 실패: {msg}',
    ...
    'live.bay_note_ended':  '기록에 없는 채로 가동하다가 이미 종료된 기체입니다(Claude Code 기록에서 실측). 기동원도 실측으로 추적하지 못했습니다.',
    'live.bay_show':       '표시',
    'live.bay_hide':       '숨기기',
    'live.orphan_parent':  '기동원 {name}',
    'card.no_mission':     '（임무 미기재）',
    'card.detected':       '보고 없음 — Claude Code 기록에서 완료 감지',
    'card.detected_stopped': '보고 없음 — Claude Code 기록에서 중단({status}) 감지',
