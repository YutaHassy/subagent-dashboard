...
- **Do not `add` units that the Workflow tool launched.** The screen finds them in Claude Code's records, places them directly under Command, and reads when they finish. Adding them afterwards under a name that differs from the label even slightly puts two cards on the screen for the same unit. Only if you want them counted in the unit total and the token total, run `add` and `done` after they finish, without `--parent` and with `--name` set to **exactly** the label shown on the screen (for example `review:security`); `done` fills in the elapsed time, tokens and tool calls from the records.
...
- **Workflow ツールが起動した機体は `add` しないでください。** 画面が Claude Code の記録から見つけて指令塔の直下に置き、終わったことも実測で読みます。終わってから `add` すると、名前がラベルと少しでも違えば同じ機体のカードが2枚になります。機体数や合計トークンに数えたいときだけ、終わったあとに `--parent` を付けず、`--name` を画面に出ているラベル（例: `review:security`）と**一字一句同じ**にして `add` と `done` を打ってください（所要・トークン・ツール回数は `done` が記録から補います）。
...
- **由 Workflow 工具启动的机体请不要 `add`。** 画面会从 Claude Code 的记录中找到它们，放在指挥部正下方，并实测读取它们何时结束。事后再 `add` 时，名字只要与标签有一点不同，同一机体就会出现两张卡片。只有想把它们计入机体数和合计 Token 时，才在它们结束后不加 `--parent`、把 `--name` 设为与画面上显示的标签（例如 `review:security`）**一字不差**，再执行 `add` 和 `done`（耗时、Token 和工具次数由 `done` 从记录中补全）。
...
- **Workflow 도구가 띄운 기체는 `add` 하지 마세요.** 화면이 Claude Code 의 기록에서 찾아 지령탑 바로 아래에 놓고, 끝난 것도 실측으로 읽습니다. 끝난 뒤에 `add` 하면 이름이 라벨과 조금이라도 다를 때 같은 기체의 카드가 두 장이 됩니다. 기체 수와 합계 토큰에 넣고 싶을 때만, 끝난 뒤에 `--parent` 를 붙이지 말고 `--name` 을 화면에 나온 라벨(예: `review:security`)과 **한 글자도 다르지 않게** 해서 `add` 와 `done` 을 실행하세요(소요 시간·토큰·도구 횟수는 `done` 이 기록에서 채웁니다).
