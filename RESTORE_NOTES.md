...
> **その原本は 0.6.0 の時点で作業ツリーから外しました。** 配る版（`agent-dashboard-0.6.0.vsix`）と
> 2つ並ぶと、受け取った人がどちらを入れるのか迷うためです。git の履歴には残っているので、
> 下の照合をやり直したいときは取り出してください。
>
> ```bash
> git show 329e1aa:agent-dashboard-0.4.3.vsix > agent-dashboard-0.4.3.vsix
> ```
>
> 同じ理由で置いてある `古い版を抜く.bat`（2,435 バイト / 0.4.3 の原本）はそのままです。
